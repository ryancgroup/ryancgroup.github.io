...
// Get Breakpoint set in main_v3.css (at the end)
...
$(document).ready(function () {
	...
	var aosEnable			= true, // ------------------------------------------------	Turn on/off AOS
		...
		aosItems			= '[class*="cool-"], .isotope .item, .posts .post', // -----	TODO : Add/remove Elements to be animated
...
// Turned this off because we are not using the carousel
		...
		submitHandler: function(form) {
			...
			if(document.getElementsByClassName('form-control error').length == 0){
				$.ajax({
					...
					{
						console.log('success');
						$('#contactform .btn-submit').html('Submit message');
						$('#commentform .btn-submit').html('Submit message');
						$('#contact-us-success').removeClass('hidden-message')
						location.hash = 'contact-us-hash';
						$('input').val('');
						$('textarea').val('');
					}
				  });
			} else {
				console.log('validation errors.  will not submit form.')
			}
			return false;
		}
